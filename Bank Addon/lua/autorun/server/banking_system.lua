...
local function CheckVGUI()
    if not vgui then
        print("VGUI is not loaded.")
    else
        print("VGUI is loaded.")
    end
end

-- Define the command
concommand.Add("checkvgui", function()
    CheckVGUI()
end)

-- Define the function to create a bank account for a player
function CreateBankAccount(ply)
  if ply.BankAccount then
    return
  end
  ply.BankAccount = {}
...
util.AddNetworkString("atm_open_menu")
